...
class SelectableScore extends Component {
  ...
  fetchAnnotationContainer() {
    auth.fetch(this.props.annotationContainerUri, {
      ...
      if (CONTAINS in container) {
        const contentUris = new Set(container[CONTAINS].map(contentItem => contentItem["@id"]));
        this.setState({
          annotationContainerContentToRetrieve: new Set(contentUris)
        }, () => {
  ...
  fetchAnnotationContainerContent() {
    this.state.annotationContainerContentToRetrieve.forEach(uri => {
      ...
      this.setState({
        annotationContainerContentRetrieved: updatedAnnotationContainerContentRetrieved
      }, () => {
        ...
        auth.fetch(uri, {
          ...
          this.setState({
            annotationContainerContentItems: { ...this.state.annotationContainerContentItems,
              [uri]: data
            } // immutable update
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      this.enableSelector();
    }, 1000); // handle fetching of annotation container contents
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.annotationContainerContentToRetrieve.size < this.state.annotationContainerContentRetrieved.size) {
      console.log("Are we done yet?", this.state.annotationContainerContentToRetrieve, this.state.annotationContainerContentRetrieved); // new content retrieved. Do we have everything?
